...
function fall() {
  // Вычисляем новую позицию мяча с учетом времени и ускорения
  position -= velocity * 0.01 + 0.5 * g * 0.0001; // 0.01 секунды - интервал анимации
  ...
  if (position <= 0) {
    // Учитываем упругость при отскоке
